...
\begin{commentbox}[Vu]
One of the reasons I create this document is that my colleagues at GMU are interested in recruiting Vietnamese students and are surprised when seeing very few applications in Vietnam (e.g., each year our CS program receives more than 350 PhD applications, most of which are international but only 3--4 are from Vietnam). In general the number of CS PhD applications from Vietnam to US universities is low and more would be very welcomed. 
\end{commentbox}
...
Admission committee will look for this part (typically written at the end of your statement), so do not skip it.
...
The following lists the minimum for GMU (being above this might not mean much, but below is a \red{red flag}).
\begin{itemize}
...
It is worth noting that if you got recruited by a prof. who would give you RA right away, it's very likely you will get admitted.  For example, if a prof., even if not in PhD admission committee, wants to work with and funds you, the PhD admission committee will respect that decision and admit you (unless your application has many red flags).
...
Faculty often receive  "cold" e-mails from international students seeking for admission, TA, and RA. Most of the time, we ignore these emails, but in some rare occasions we do answer them. So how to write an email that can get our attention?
...
A related topic is to consider is how to refer to a professor you know. Here, because you are already familiar with these individuals, you can informally use their firstname (or Dr./Prof., if you want to be formal) as mentioned above. You can also include their institution if it makes it more precise.  For example, I did my postdoc with Jeff Foster at Univ. of Maryland.   Do not include ranking (e.g., Assistant, Associate, Scientist, ...) when referring to someone. I see many students, especially Vietnamese, include a lengthy title of people they know, e.g., my advisors are Asst. Prof. X and Assoc. Prof. Y., and I also collaborate with ACM Distinguished Scientist Z.  This is \emph{not necessary} and makes it look like you're trying to show off your connections. In short, these nuances represent some cultural differences that you may encounter and will gradually adapt to (more on cultural differences in \S\ref{sec:cultural}).
...
\paragraph{Diversity} US universities prioritize diversity and inclusion. Students need to respect and appreciate varied opinions, backgrounds, and cultures. Unlike some countries where certain voices are ignored, in the US, all perspectives are valued equally (especially at universities). Racism or discrimination will have serious consequences, including academic and disciplinary actions.
...
Profs. Craig Yu (GMU), Hakan Aydin (GMU), and Hung Le (UMass) provided valuable input in the early version. Other GMU faculty members also have provided feedback and contributions.  Many students including Didier (GMU), Thanh (Melbourne), and Dat (Melbourne) have contributed valuable questions and feedback. 
\textbf{Thank you!}
