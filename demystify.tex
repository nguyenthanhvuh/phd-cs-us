...
% \section{How much time did you spend looking at an application?}\label{sec:time-spent}
% Short answer: less than 5 mins for quick rejection; 15+ mins for an acceptance.  On average, about 10 mins.

% The GMU evaluation system allows us to download a single pdf that consists of a summary page (e.g., school info, GPA, testscores), then transcripts, standard test results, LoRs, CV, SOP, writing samples So I typically read the application in this order.  Then I make notes on what I've seen, basically evidence that support my evaluation.

% I first look at the summary for school, GPA, est scores (essentially looking for very low GPA or testscores that do not meet the minimum requirements. Then I skim on transcripts look for low/failing grades on CS and related courses.  For these I make comments such as "unknown international school with good GPA, too many low grades on CS and related courses" or "good GPA from top school XYZ". I do not look at details on tests (the main scores are already given in the first summary page).


% Then I look at LoRs.  I read carefully good LoRs and quickly skim over bad or generic ones (very easy to tell, you don't even need much experiences to distinguish between a strong LoRs from the rest). Comment examples For LoRs include "very strong LoRs from well-known prof. XYZ talking about research experiences" or "generic LoRs, mostly about course work".  I also note any unusual things in the LoRs.

% Then the SOP, which I read carefully (of course if it is bad then it would a quick read). Then my notes for SOP include things such as "SOP mostly on course work, no clear motivation, generic" , or "very enjoyable SOP talking about XYZ, customized for GMU, mentioned profs. X,Y,Z".  If the SOP mentioned about publications then I'll make notes such as "has X papers at top venues" or "has X papers but at unknown places".  I will not go online search for the papers (unless they upload them as sample papers) .

% I also skim over sampling papers, essentially looking over the abstract to see if reads well.  I would make notes such as "have a paper under submission to unknown venue, but reads well".
...
%At GMU we use an evaluation system that allows us to download a single, large pdf that consists of everything submitted by the applicant, e.g., GPA, transcripts, test scores, CV, LoRs, SOP, sample writings, etc.   The first couple of pages show the summary including school, major, GPA, test scores.
