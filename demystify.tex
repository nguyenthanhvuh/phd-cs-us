...
\begin{enumerate}
  ...
  \item Application Materials
        \begin{itemize}
          ...
          \item  \emph{Pubication can greatly help}. Publications are good venues are concrete evidence that you have successfully engaged in research. But they are not required! (\autoref{sec:research-experiences}).
...
Many possible reasons!  Other than not having a strong profile (e.g., research potentials, GPA, LoRs, SOP, poor interviews), here are some that you might not expect.
...
Similarly, if there is no faculty who is willing to advise you (e.g., they are on sabbatical, personal leave, do not have sufficient fundings, or already have too many students already), you will not be admitted.  This is actually good for you, as you don't want to be in a program where no one can advise you.
...
\paragraph{You aim too high}  You might have applied to schools that are too high for your profile. While being ambitious is good, you should also be realistic and apply to a range of schools, including ``safety'' ones.  I have seen many students who have excellent profiles but only applied to top 10 or even top 20 schools and got rejected from all of them.  Before applying, you should talk to your professors and ask them to give you an honest opinion on where you should apply

\paragraph{Overqualified or Lack Interest}  This might be paradoxical, however if the adcom believes that you are likely to get admitted and go to a better program (i.e., they are just your ``safety'' schools), they might not be so enthusiastic to admit you and want to save the spot for someone else. Note though it does happen, for most good schools this is rare and you should not assume this is the case.
...
\paragraph{Red flags in Your Background} Plagiarism, cheating, or other academic dishonesty are \red{red} flags. Another one is if you have a history of jumping from one program to another (e.g., you have been in multiple different programs in short time). If you think you have red flags, you should address them in your SOP.
...
\section{How much time did you spend looking at my application?}\label{sec:time-spent}

Around 15 minutes on average (less than 10 mins for definite rejection; 20+ mins for an acceptance). Note I spent more time in my earlier years, but now I have more experience and can quickly tell if an application is good or bad. 

The GMU evaluation system allows us to download a single pdf that consists of a summary page (e.g., school info, GPA, standard test scores), then transcripts, test results, LoRs, CV, SOP, writing samples (if any). So I typically scan the application in this order.  I also make notes on what I've seen, basically evidence that support my evaluation.

I first skim the summary for school, GPA, test scores to looking for red flags like low GPA or test scores that do not meet the minimum requirements. Then I move to transcripts to look for low/failing grades on CS and related courses.  For these I make comments such as ``- unknown international school with good GPA, too many low grades on CS and related courses" or ``+ very good GPA from top school XYZ". I do not look at test details as the summary already gives the main scores.
...
At this point I have enough to make up my decision, which I enter to the evaluation system. We have several options for decision, but typically I just use \emph{reject} and \emph{admit with full funding}.  For the latter, I also mention funding sources (e.g., RA from prof. X or TA). I can also recommend very strong students for the University Presidential fellowship.  Finally, the system has text box where I put my comments to support my decision.

After I submit my evaluation, the adcom chair will look all evaluations for that application and make a decision (almost always following our suggestion).  If there are large discrepancies such as one vote reject but another full funding acceptance (such cases are rare, but do happen), the chair might ask us to coordinate and re-evaluate the application if necessary.
