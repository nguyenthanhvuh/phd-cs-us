  ...
  \item Choosing School and Professors
    ...
    \item Miscs and FAQS
        \begin{itemize}
          ...
          \item Sometimes adcom members do dicuss applicants, but in most cases they make independent decisions (\autoref{sec:adcom-discuss}).           
          \item Being a minority can help (\autoref{sec:urm}).
...
In most cases we make decision independently and do not talk to each other.
However, we do discuss when there are discrepancies in evaluations, in which case the adcom chair will ask reviewers discuss the application to reach a consensus.  We also chat to each other for interesting or strong applications that we think should be admitted, e.g., how to recruit this student or who should be the adviser. If the student mentioned a faculty member in their SOP, we might ask that faculty member if they are interested in the student.  In short, while we do not talk to each other for most applications, we do for some.
